...
// ─── NAVIGATION ────────────────────────────────────────────────
const VIEWS = ['dashboard','analysis','portfolio','backtest','model'];
function switchView(view) {
  ...
  if (view==='backtest')  initBacktestView();
  if (view==='model')     initModelView();
}
...
function chartOpts(legend=true) {
  return {
    ...
    plugins: {
      ...
      tooltip: { backgroundColor:'#141829', borderColor:'rgba(255,255,255,0.08)', borderWidth:1,
...
function runBacktest() {
  ...
  for (let i=14; i<prices.length; i++) {
    ...
    if (signal==='buy' && !position) {
      ...
      equity += pnl - fee;
      trades.push({ day:i, type:'SELL', price:p, shares:position.shares,
